...
describe('workspace CLI', () => {
  it('parses roots and defaults', () => expect(parseArgs(['--root=/tmp/project', '--allow-write'])).toEqual({
    roots: ['/tmp/project'], allowWrite: true, http: false, host: '127.0.0.1', port: 3000, toolProfile: 'all', config: undefined,
  }));
  ...
  it('parses tool profiles', () => expect(parseArgs(['--tool-profile=review'])).toMatchObject({ toolProfile: 'review' }));
  it('rejects missing, relative, and rootless write options', () => {
    ...
    expect(() => parseArgs(['--tool-profile', 'unknown'])).toThrow(/review, convert, structure, workspace, all/);
  });
});
